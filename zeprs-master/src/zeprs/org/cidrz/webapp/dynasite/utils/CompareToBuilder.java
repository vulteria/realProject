...
/**
 * CEKelley: Modifications to display which comparisons failed.
 *
 ...
 * @see EqualsBuilder
 * @see HashCodeBuilder
 * @author <a href="mailto:<a href="/cdn-cgi/l/email-protection" class="__cf_email__" data-cfemail="0c7f78697a692268637b6269754c6269786a63606563226f6361">[email protected]</a>">Steve Downey</a>
...
public class CompareToBuilder {
  ...
  /*  public static int reflectionCompare(Object lhs, Object rhs, Collection String excludeFields) {
     ...
     * @return
     */
    public static ArrayList reflectionCompareResults(
    		Object lhs,
    		Object rhs,
    		boolean compareTransients,
    		Class reflectUpToClass,
    	...
    	CompareToBuilder compareToBuilder = new CompareToBuilder();
    	results = reflectionAppendResults(lhs, rhs, lhsClazz, compareToBuilder, compareTransients, excludeFields);
    	while (lhsClazz.getSuperclass() != null && lhsClazz != reflectUpToClass) {
    		lhsClazz = lhsClazz.getSuperclass();
    		reflectionAppendResults(lhs, rhs, lhsClazz, compareToBuilder, compareTransients, excludeFields);
    	}
    	// return compareToBuilder.toComparison();
    	return results;
    }
     ...
     * @return
     */
    private static ArrayList reflectionAppendResults(
    		Object lhs,
    		Object rhs,
    		Class clazz,
    		CompareToBuilder builder,
    	...
    	String classNameShort = className.replace("org.cidrz.project.zeprs.valueobject.gen.", "");
    	for (int i = 0; i < fields.length && builder.comparison == 0; i++) {
    		Field f = fields[i];
    		if (!excludedFieldList.contains(f.getName())
    				&& (f.getName().indexOf('$') == -1)
    			...
    			try {
    				builder.append(f.get(lhs), f.get(rhs));
    				if (builder.comparison != 0) {
    					String message = classNameShort + " : " + f.getName() + " : " + f.get(lhs).toString() + " is not equal to " + f.get(rhs);
    					results.add(message);
    				}
    				//this.evaluate(f.get(lhs), f.get(rhs), null);
    			} catch (IllegalAccessException e) {
    				// This can't happen. Would get a Security exception instead.
    				// Throw a runtime exception in case the impossible happens.
    				throw new InternalError("Unexpected IllegalAccessException");
     ...
     * @return
     */
    public int evaluate(Object lhs, Object rhs, Comparator comparator) {
    	int comparison = 0;
    	if (lhs == rhs) {
    		return comparison;
    	}
